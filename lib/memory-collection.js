...
function validId(id) {
  return typeof id === 'string' || (id && typeof id === 'object' && typeof id.$oid === 'string' && id.$oid.length === 24);
}
function idEqual(left, right) {
  if (!validId(left) || !validId(right)) return false;
  if (typeof left === 'string') return left === right;
  else return left.$oid === right.$oid;
}
...
Collection.prototype.insert = function (document) {
  ...
  if (!validId(document._id)) {
    return Promise.reject(new TypeError('mongod-mock requires IDs to be strings or ObjectIds.'));
  }
  if (this.data.some(function (doc) { return idEqual(doc._id, document._id); })) {
...
Collection.prototype.update = function (query, update, options) {
  options = options || {};
  if (options.upsert && !idEqual(query._id, update._id)) {
